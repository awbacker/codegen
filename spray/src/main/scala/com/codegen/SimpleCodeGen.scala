...
import com.codegen.data.activate.CodeType
import com.codegen.data.activate.Context._
...
class SimpleCodeGen {
    ...
    def get_data() = {
        transactional {
            val codeType = select[CodeType] where(_.id :== 1)
            println(codeType)
            codeType
        }
    }
    ...
    implicit def padLeft(s: String, len: Int, char: Char) = {
    }
